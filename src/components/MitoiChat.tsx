...
import { Bot, X, Send, Loader2, RotateCcw, Sparkles, CheckCircle2, XCircle, Trophy } from 'lucide-react';
...
export default function MitoiChat() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      addAssistantMessage(
        `Hey${name}! 👋 I'm **Mitoi AI**, your AuraSkill AI companion.\n\nI can:\n• 🧭 Navigate anywhere\n• 🧠 Quiz you (aptitude, logical, coding)\n• 💡 Answer platform questions\n\nTry: *"Start a quiz"* or *"Go to career planner"*`
      );
    }
  ...
  const handleSend = async () => {
    ...
    if (!text || isLoading || quizLoading) return;
        ...
        {isOpen && (
          ...
          >
            <Card className="w-[380px] sm:w-[420px] h-[620px] flex flex-col shadow-2xl border-2 border-teal-200/50 dark:border-teal-800/50 overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-teal-500 to-cyan-600 text-white">
                <div className="flex items-center gap-2.5">
                  <div className="h-9 w-9 rounded-full bg-white/20 flex items-center justify-center">
                    <Bot className="h-5 w-5" />
                  </div>
                  <div>
                    <h3 className="font-bold text-sm tracking-wide">Mitoi AI</h3>
                    <p className="text-[10px] opacity-80">AuraSkill AI Assistant</p>
                  </div>
                </div>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
                    <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-sm rounded-2xl rounded-bl-md px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400">Mitoi is thinking...</span>
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="p-3 border-t dark:border-gray-800 bg-white dark:bg-gray-900">
                <div className="flex gap-2">
                  <Input
                    ...
                    placeholder={quiz?.active ? "Type A, B, C, or D..." : "Ask Mitoi anything..."}
                    disabled={isLoading || quizLoading}
                    className="flex-1 text-sm dark:bg-gray-800 dark:border-gray-700 rounded-full px-4"
                  />
                  <Button
                    onClick={handleSend}
                    disabled={!input.trim() || isLoading || quizLoading}
                    size="icon"
                    className="rounded-full bg-gradient-to-br from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 shrink-0"
                  >
