...
// All requests are proxied through the Vite server (/api/judge0/*).
// The real Judge0 host lives in JUDGE0_HOST (.env server-side only) – never exposed to the browser.
...
export const executeCode = async (
  ...
  // All execution is proxied through the Vite API server.
  // The real Judge0 host (JUDGE0_HOST) is read server-side and never reaches the browser.
  const proxyBase = '/api/judge0';
