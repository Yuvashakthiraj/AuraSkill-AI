...
export const ResumeUpload = ({ roleId, onResumeProcessed, minimumScore = 60, showBestMatch = false }: ResumeUploadProps) => {
  const { user } = useAuth();
  const { setResume: saveToResumeContext } = useResume();
        ...
        if (user) {
          try {
            ...
            if (success) {
              ...
              // Save to centralized cache for "Upload Once, Use Everywhere"
              saveToResumeContext(cleanedResume, processedResume);
